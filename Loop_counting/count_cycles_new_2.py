    ...
    if(test==False):
        if(graph_algo=='Gusev'):
            ...
            ##   N = 12   
              ...
              if(ite==-1):
                  filename = "2_network_KMC.txt"
              else:            
                  filename = "2_network_KMC_"+str(ite)+".txt"
              file_path = os.path.join(directory, filename)
              if not os.path.isdir(directory):
                 os.mkdir(directory)  
            ##  [xlo, xhi, ylo, yhi, zlo, zhi, n_atoms, n_bonds, atoms, bonds, 
...
if __name__=='__main__':
    ...
    ite_arr=np.arange(0, 4*NRA,NRA/10,dtype='int')
...
##    stop
    ...
    ##    plt.plot(np.arange(1,max_loop_order+1), (loop_order_count_mean_field[1:]),'bo-') # check this ceil() function!!- i am assuming this is the correct implementation, but not sure
        ...
        plt.savefig('2_mean_field_loop_order_dist_new_'+str(ite),bbox_inches='tight')
        np.savetxt('2_loop_order_mean_field_new'+str(ite)+'.txt',np.array(np.transpose([np.arange(1,max_loop_order+1), (loop_order_count_mean_field[1:]),(loop_fraction_mean_field[1:])])),header='loop_order,count,fraction')
